...
// resolve : 성공
// reject : 실패
//  프로미스는 콜백함수 전달할 필요 없음. 그냥 호출하면 됨.
...
function promise1(flag) {
  ...
  return new Promise(function (resolve, reject) {
    // (resolve, reject) apple, banana 등 다른 것도 가능. 그러나 (resolve, reject)가 제일 직관적임.
    if (flag) {
      resolve(
        `promise 상태는 fulfiled!! then으로 연결됩니다. 이때의 flag는 ${flag}입니다.`
      );
    } else {
      reject(
        `promise 상태는 rejected!! catch으로 연결됩니다. 이때의 flag는 ${flag}입니다.`
      );
    }
  });
}

// 2. promise를 사용(소비)하느 코드
promise1(5 < 3) // 맞는 함수를 사용하면 콘솔에 true, 틀린 함수 사용하면 false
  ...
  */
//-------------------------------------------------------------------------------------
// index.js 에서 '콜백함수'를 이용해 동기처리한 것을 promise를 이용해서 동기처리 해보자

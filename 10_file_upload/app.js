...
const upload = multer({
  dest: "uploads/", //  dest: 업로드할 파일 경로 지정 (여기서 슬래시(/)는 폴더를 의미)
});
const uploadDetail = multer({
  storage: multer.diskStorage({
    ...
    filename(req, file, done) {
      // ex) apple.png 파일 업로드
      const ext = path.extname(file.originalname); // file.originalname 에서 '확장자' 추출 => ex) png
      // path.basename(file.originalname, ext) => apple (확장자 제거한 파일 이름만!)
      // Date.now() => 현재시간 (1680309346279)
      // (1680309346279) => 1970년 1월 1일 0시 0분 0초를 기준으로 현재까지 경과된 밀리초
...
// single()의 매개변수: input의 name과 일치 시키기. <input type="file" name="userfile" />
app.post("/upload", uploadDetail.single("userfile"), (req, res) => {
  console.log(req.file);
  // 업로드한 파일 정보
  /* {
    fieldname: 'userfile', // 폼에 정의한 name
    originalname: 'puppy1.jpeg', // 사용자가 업로드한 파일명
    encoding: '7bit', // 파일 인코딩 타입
    mimetype: 'image/jpeg', // 파일 mime 타입
    destination: 'uploads/', // 파일 저장할 경로(폴더)
    filename: '35d26f11c24d5a6fc12d660166c3824d', // destination에 저장된 파일명
    path: 'uploads/35d26f11c24d5a6fc12d660166c3824d', // 업로드된 파일의 전체 경로
    size: 134058 // 파일 크기 (byte)
  } */

  console.log(req.body);
  // 폼에 입력한 정보
  /* { title: '강아지1' } // input title 에 입력한 값 */
...
// fileds(): 여러 파일을 각각 input에 업로드할 때 사용
app.post(
  "/upload/fields",
